...
from keras.layers import Input,Dense,LSIM ,Embedding, Dropout
...
print("length of va=ocabulary =",len(descriptions))
...
def load_features(photos):
    all_features = load(open("feactures.p","rb"))
    features ={k:all_features[k] for  k in photos}
    return features

filename = dataset_text + "/"+ "flickr8k.trainImages.txt"
    ...
    def generator():
        while True:
            for key, description_list in descriptions.items():
                ...
                input_image , input_sequence , output_word = create_sequences(tokenizer,max_length,descriptions)
...
def create_sequences(tokenizer,max_length,desc_list,feature):
    X1,X2,y= list(),list(),list(),list()
    for desc in desc_list:
        seq = tokenizer.texts_to_sequences([desc])[0]
        for i in range (1,len(seq)):
            in_seq,out_seq =seq[:1],seq[i]
            in_seq =pad_sequences([in_seq],maxlen= max_length)[0]
            out_seq = to_categorical([out_seq],num_classe = vocab_size)[0]
            X1.append(feature)
            X2.append(in_seq)
            y.append(out_seq)
...
def define_model(vocab_size,max_length):
    ...
    fe2 = Dropout(256,activation='relu')(fe1)

    #LSIM sequences model
